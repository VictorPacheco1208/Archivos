...
void mostrar(int n, char arreglo[5]){
    int i, j;
    for(j=0; j<n; j++){
        printf("Arreglo numero %i\n", j+1);
        for(i=0; i<5; i++){
            printf("Caracter numero [%i]: %c\n",i+1, arreglo[i]);   
        }
        printf("\n");
    }
}
...
int main(){
    ...
    do{
        ...
        switch(opc){
            ...
            case 2:
                printf("¿Cuantas veces quieres que el arreglo se repita? ");
                    scanf("%i", &b);
                for(i=0; i<5; i++){
                    char actual;
                    printf("Coloca solo un caracter %i: \n", i+1);
                    scanf(" %c", &actual);
                    a[i] = actual;
                }
                mostrar(b, a);
